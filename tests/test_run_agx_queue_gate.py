import hashlib
import json
import os
from pathlib import Path
import subprocess
import sys
import tempfile
import unittest

from tools.agx_contract import load_contract
from tools.artifact_manifest import ARTIFACT_ROLES, J313_GUEST_CONTRACT
from tests.agx_gate_test_support import install_contract_git


ROOT = Path(__file__).resolve().parents[1]
SCRIPT = ROOT / "scripts" / "run-agx-queue-gate.sh"
CONTRACT_PATH = ROOT / "config" / "j313-agx.json"
ARTIFACT_NAMES = tuple(ARTIFACT_ROLES)


def sha256(path):
    return hashlib.sha256(path.read_bytes()).hexdigest()


class RunAgxQueueGateTests(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.base = Path(self.tmp.name)
        self.artifacts = self.base / "stable"
        self.artifacts.mkdir()
        self.evidence = self.base / "evidence"
        self.test_bin = self.base / "bin"
        self.test_bin.mkdir()
        fake_ps = self.test_bin / "ps"
        fake_ps.write_text("#!/bin/sh\nexit 0\n")
        fake_ps.chmod(0o755)
        contract = load_contract(CONTRACT_PATH)
        install_contract_git(self.test_bin, contract)
        records = {}
        checksum_lines = []
        for index, name in enumerate(ARTIFACT_NAMES, 1):
            path = self.artifacts / name
            path.write_bytes((f"fixture-{index}-{name}\n").encode())
            digest = sha256(path)
            records[name] = {
                "size": path.stat().st_size,
                "sha256": digest,
                "role": ARTIFACT_ROLES[name],
            }
            checksum_lines.append(f"{digest}  {name}\n")
        manifest = {
            "format_version": 2,
            "platform": "j313",
            "profile": "debug",
            "display": "both",
            "debug": "monitor",
            "compiler": "test compiler",
            "guest_layout_sha256": "a" * 64,
            "guest_contract": J313_GUEST_CONTRACT,
            "root_commit": contract.source.root_commit,
            "source_dirty": False,
            "m1n1_windows_commit": contract.source.m1n1_commit,
            "m1n1_windows_dirty": False,
            "mu_commit": contract.source.mu_commit,
            "mu_dirty": False,
            "artifacts": records,
        }
        (self.artifacts / "MANIFEST.json").write_text(
            json.dumps(manifest, indent=2, sort_keys=True) + "\n"
        )
        (self.artifacts / "SHA256SUMS").write_text("".join(checksum_lines))
        self.protected = [
            ROOT / "scripts" / "run-assisted.sh",
            ROOT / "scripts" / "build-standalone.sh",
            ROOT / "launch_profile.py",
            ROOT / "standalone_image.py",
        ]

    def tearDown(self):
        self.tmp.cleanup()

    def command(self, *extra):
        return [
            "sh",
            str(SCRIPT),
            "--proxy",
            "/dev/cu.test-proxy",
            "--contract",
            str(CONTRACT_PATH),
            "--artifact-dir",
            str(self.artifacts),
            "--evidence-dir",
            str(self.evidence),
            "--cycles",
            "10",
            *extra,
        ]

    def run_script(self, *extra):
        env = os.environ.copy()
        env["PATH"] = f"{self.test_bin}{os.pathsep}{env['PATH']}"
        return subprocess.run(
            self.command(*extra),
            cwd=self.base,
            env=env,
            capture_output=True,
            text=True,
            check=False,
        )

    def assert_protected_unchanged(self, action):
        before = {path: sha256(path) for path in self.protected}
        result = action()
        after = {path: sha256(path) for path in self.protected}
        self.assertEqual(after, before)
        return result

    def test_dry_run_prints_the_literal_g1q_contract(self):
        result = self.assert_protected_unchanged(
            lambda: self.run_script("--dry-run")
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        for boundary in (
            "mode: assisted AGX G1Q queue gate",
            "context: 63",
            "queue: 3D index 1",
            "commands per cycle: 1",
            "completion deadline: 0.5 seconds",
            "cycles: 10",
            "reset policy: physical cold reset after every cycle",
        ):
            self.assertIn(boundary, result.stdout)

    def test_timeout_option_is_not_exposed(self):
        result = self.assert_protected_unchanged(
            lambda: self.run_script("--timeout", "1", "--dry-run")
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertNotIn("--timeout", result.stdout)

    def test_cycles_other_than_ten_are_rejected(self):
        command = self.command("--dry-run")
        command[command.index("10")] = "9"
        result = self.assert_protected_unchanged(
            lambda: subprocess.run(command, capture_output=True, text=True, check=False)
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("--cycles must be exactly 10", result.stderr)

    def test_nonempty_evidence_is_rejected_even_for_dry_run(self):
        self.evidence.mkdir()
        (self.evidence / "stale.json").write_text("{}")
        result = self.assert_protected_unchanged(
            lambda: self.run_script("--dry-run")
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Evidence directory is not empty", result.stderr)

    def test_missing_manifest_is_rejected(self):
        (self.artifacts / "MANIFEST.json").unlink()
        result = self.assert_protected_unchanged(
            lambda: self.run_script("--dry-run")
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Artifact manifest not found", result.stderr)

    def test_changed_artifact_is_rejected(self):
        (self.artifacts / "m1n1.macho").write_bytes(b"modified")
        result = self.assert_protected_unchanged(
            lambda: self.run_script("--dry-run")
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("artifact hash mismatch", result.stderr)

    def test_unexpected_recovery_entry_is_rejected(self):
        (self.artifacts / "unexpected.bin").write_bytes(b"dirty")
        result = self.assert_protected_unchanged(
            lambda: self.run_script("--dry-run")
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("unexpected recovery entry", result.stderr)

    def test_active_guest_is_rejected(self):
        pid_path = ROOT / "guest.pid"
        previous = pid_path.read_bytes() if pid_path.exists() else None
        try:
            pid_path.write_text(f"{os.getpid()}\n")
            result = self.assert_protected_unchanged(
                lambda: self.run_script("--dry-run")
            )
        finally:
            if previous is None:
                pid_path.unlink(missing_ok=True)
            else:
                pid_path.write_bytes(previous)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("guest runner", result.stderr)

    def test_false_aggregate_is_rejected(self):
        path = self.base / "queue-gate-result.json"
        data = {
            "queue_gate_version": 2,
            "requested_cycles": 10,
            "completed_cycles": 10,
            "cycles": [],
            "cold_reset_between_cycles": True,
            "verdict": "failed",
            "windows_launch_permitted": False,
            "aggregate_sha256": "0" * 64,
        }
        path.write_text(json.dumps(data))
        result = self.assert_protected_unchanged(
            lambda: subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "tools.agx_queue_gate",
                    "verify-result",
                    str(path),
                ],
                cwd=ROOT,
                capture_output=True,
                text=True,
                check=False,
            )
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("aggregate_sha256", result.stderr)

    def test_failed_cycle_path_reboots_before_exit(self):
        source = SCRIPT.read_text()
        run_one = source.index("tools.agx_queue_gate run-one")
        reboot = source.index("proxyclient/tools/reboot.py", run_one)
        failed_exit = source.index('if [ "$CYCLE_OK" -ne 1 ]', reboot)
        self.assertLess(run_one, reboot)
        self.assertLess(reboot, failed_exit)

    def test_runner_never_substitutes_the_g1_gate(self):
        source = SCRIPT.read_text()
        self.assertNotIn("tools.agx_gate run-one", source)
        self.assertIn("tools.agx_queue_gate run-one", source)


if __name__ == "__main__":
    unittest.main()
