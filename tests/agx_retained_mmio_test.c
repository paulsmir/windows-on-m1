...
int main(void) {
  ...
  AGX_RR_REQUEST q = {AGX_RR_ABI_VERSION,64,AGX_RR_MAP,0,1,17,0xffffffa010000000ULL,0x200000,0x4000,0};
  ...
  assert(hv_agx_retained_mmio(&state,64,&value,1,2,execute,0));
  assert(calls==1 && state.Response.Status!=0);
  state.Request.Version=AGX_RR_ABI_VERSION; state.Request.Bytes=63;
  assert(hv_agx_retained_mmio(&state,64,&value,1,2,execute,0));
  assert(calls==1);
