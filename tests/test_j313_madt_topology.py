...
class TestJ313MadtTopology(unittest.TestCase):
    def test_cpu_stability_experiment_exposes_four_efficiency_and_two_performance_cores(self):
        source = PUBLIC_MADT.read_text()

        self.assertEqual(enabled_gicc_uids(source), [0, 1, 2, 3, 4, 5])

        classes = gicc_efficiency_classes(source)
        self.assertEqual([classes[uid] for uid in range(4)], [0, 0, 0, 0])
        self.assertEqual([classes[uid] for uid in (4, 5)], [1, 1])
