...
class AppleAgxRenderAdmissionTests(unittest.TestCase):
    ...
    def test_interrupt_admission_owns_only_synthetic_broker_status_and_ack(self):
        ...
        self.assertNotIn(
            "_Use_decl_annotations_ VOID AdmissionDdiDpcRoutine", interrupt
        )
    ...
    def test_project_links_only_admission_and_selected_provider_sources(self):
        ...
        for provider in (
            "apple_agx_backend_runtime.c",
            "apple_agx_g13_queue_provider.c",
            "apple_agx_platform_provider.c",
        ):
            self.assertIn(provider, project)
        for forbidden in (
            "m1n1", "render_job", r"..\windows\src",
        ):
