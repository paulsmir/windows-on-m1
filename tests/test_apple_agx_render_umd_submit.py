...
class AppleAgxRenderUmdSubmitTests(unittest.TestCase):
    ...
    def test_producer_uses_current_non_test_d3dkmt_contract(self):
        ...
        self.assertIn("createContext.Flags.GdiContext = 1u", source)
        self.assertIn("D3DKMT_RENDER render = {0}", source)
        self.assertIn("DXGK_RENDERKM_COMMAND command = {0}", source)
        self.assertIn("render.Flags.RenderKm = 1u", source)
        self.assertIn("render.pNewCommandBuffer = &command", source)
        self.assertIn("render.pNewAllocationList = &allocationList", source)
