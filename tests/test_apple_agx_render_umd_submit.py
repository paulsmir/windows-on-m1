...
class AppleAgxRenderUmdSubmitTests(unittest.TestCase):
    ...
    def test_producer_uses_current_non_test_d3dkmt_contract(self):
        ...
        self.assertEqual(source.count("D3DKMTLock2("), 2)
        self.assertEqual(source.count("D3DKMTUnlock2("), 2)
        self.assertIn("APPLE_AGX_OUTPUT_INITIAL_PATTERN", source)
        self.assertIn("APPLE_AGX_EXP208_GDI_COLOR", source)
        self.assertIn("output_pixels_verified", source)
        self.assertIn("output_first_pixel_actual", source)
        self.assertIn("output_first_mismatch_actual", source)
        self.assertIn("output_pixels_still_a5", source)
        self.assertIn("output_changed_bytes", source)
        self.assertIn("output_target_fnv1a", source)
        self.assertIn("OUTPUT_PREFIX", source)
        self.assertIn("output_guard_corrupt", source)
