...
class AppleAgxWindowsPackageTests(unittest.TestCase):
    ...
    def test_lifecycle_cycle_is_hash_pinned_and_device_scoped(self):
        ...
        self.assertEqual(cycle.count('@("/scan-devices")'), 1)
        self.assertNotIn('@("/restart-device", $deviceId)', cycle)
        clear_receipts = cycle.rindex("Clear-LifecycleReceipts")
        install_driver = cycle.index('@("/add-driver", $inf, "/install")')
        self.assertLess(clear_receipts, install_driver)
