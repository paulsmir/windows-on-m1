...
def load_g2_contract(g2_path=G2_CONTRACT, g1_path=G1_CONTRACT):
    ...
    if data["contract_version"] != 3:
        raise G2ContractError("contract_version must be 3")
    ...
    if (not isinstance(mmio_names, list)
        ...
        raise G2ContractError(
            "ACPI MMIO regions must be exactly sgx_mmio then gpu"
        )
    acpi_mmio = tuple(
...
def render_asl_include(contract):
    if len(contract.acpi_mmio) != 2:
        raise G2ContractError("ASL requires the SGX aperture and gpu-region")
    ...
    mmio = "\n".join(
        _qword_memory(base, size) for _, base, size in contract.acpi_mmio
    )
...
def render_m1n1_header(contract):
    if len(contract.acpi_mmio) != 2:
        raise G2ContractError("m1n1 policy requires the SGX aperture and gpu-region")
    mmio = {name: (base, size) for name, base, size in contract.acpi_mmio}
    base, size = mmio["sgx_mmio"]
    gpu_base, gpu_size = mmio["gpu"]
    if len(contract.synthetic_mmio) != 1:
    ...
    lines = [
        ...
        f"#define HV_AGX_G2_GPU_BASE 0x{gpu_base:x}ULL",
        f"#define HV_AGX_G2_GPU_SIZE 0x{gpu_size:x}ULL",
